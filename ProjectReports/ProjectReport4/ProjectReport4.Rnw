...
We seek to compare the relative success and reception of various book, e-book, and audiobook publishers at the Seattle Public Library from 2005 to 2022, so as to better understand the general popularity of such publishers in the literary marketplace.
...
As a baseline for comparison, we first isolate the total monthly checkouts for books, e-books, and audiobooks from 2005 to 2022 as shown below in Figure \ref{fig:1}.
